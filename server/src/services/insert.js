    ...
    try {
        ...
        dataBody.forEach(cate => {
            cate.body.forEach(async (item) => {
                let postId = v4()
                let labelCode = generateCode(item?.header?.class?.classType).trim() // trim () dùng để cắt dấu cách
                ...
                await db.Overview.create({
                    ...
                    target: item?.overview?.content.find(i => i.name === "Đôi tượng thuê:")?.content,
